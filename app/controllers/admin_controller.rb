class AdminController < ApplicationController
  ...
  def transaction_report
    ...
    @transactions = BankTransaction.where("date(transaction_time) >= ? AND date(transaction_time) <= ?", params[:start_date], params[:end_date]).where(bank_account_id: account_ids).includes(:bank_account).order(:bank_account_id)
